...
let pretty_print e = 
  let rec print = function
      And (e1, e2) -> print_string "("; print e1; print_string " ∧ "; print e2; print_string ")"
    | Or (e1, e2) -> print_string "("; print e1; print_string " ∨ "; print e2; print_string ")"
    | Variable (s) -> Printf.printf "'%s'" s
    | LogicalValue (b) -> Printf.printf "%s" (match b with true -> "true" | _ -> "false")
    | Not(e) -> print_string "¬"; print_string "("; print e; print_string ")"
  in print e; print_endline ""
...
and optimize_not = function
  ...
  | Not (e) -> (
      let oe = optimize e 
      in match oe with LogicalValue _ -> (optimize_not (Not (oe))) | _ -> Not (oe)
    )
  | _ -> failwith "Can't happen"
...
let rec df_satisfied e = 
  ...
  | Some s -> (
      ...
      and exp_false = assign_variable s false e |> optimize
      in (df_satisfied exp_true) || (df_satisfied exp_false)
    )
